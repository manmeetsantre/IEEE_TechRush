...
def generate_mcqs(text, count, difficulty, chapter, provider):
    ...
    # Clear prompt with explicit JSON format request
    prompt = f"""
Create {count} multiple choice questions based on this text extracted from PDF:\n\n{text}
Requirements:
- difficulty: {difficulty}
- chapter: {chapter}
...
#                 response_text = generate_with_ollama(prompt)
#                 if not response_text: 
#                     error_messages.append("Ollama returned empty response")
#             except Exception as e:
#                 error_messages.append(f"Ollama failed: {e}")
        
#         if (provider == 'gemini') or (provider == 'ollama' and not response_text):
#             try:
#                 response_text = generate_with_gemini(prompt)
#             except Exception as e:
#                 error_messages.append(f"Gemini failed: {e}")

#         # If Ollama failed or we're using Gemini
#         if not response_text and provider == 'gemini' or error_messages:
#             try:
#                 print("Falling back to Gemini due to issues with Ollama")
#                 response_text = generate_with_gemini(prompt)
#             except Exception as e:
#                 error_messages.append(f"Gemini failed: {e}")
        
#         if not response_text:
#                     error_msg = f"Batch {i//batch_size + 1} failed: {', '.join(error_messages)}"
#                     print(error_msg)
...
#     #     if json_match:
#     #         batch_mcqs = json.loads(json_match.group(0))
#     #     else:
#     #         batch_mcqs = json.loads(response_text)

#     #     for idx, val in enumerate(batch_mcqs):
#     #         val['id'] = len(final_response_json) + idx + 1
#     #         if isinstance(val['correctAnswer'], str):
#     #             val['correctAnswer'] = int(val['correctAnswer'])

#     #     final_response_json.extend(batch_mcqs)
...
#                     batch_mcqs = json.loads(json_match.group(0))
#                 except json.JSONDecodeError as e:
#                     error_msg = f"JSON parse error after regex: {e}"
#                     print(error_msg)
#                     error_messages.append(error_msg)
#                     continue
#             else:
#                 error_msg = "No JSON array found in response"
#                 print(error_msg)
#                 error_messages.append(error_msg)
#                 continue

#         # Validate and process the MCQs
#         valid_mcqs = []
#         for idx, mcq in enumerate(batch_mcqs):
#             if not isinstance(mcq, dict):
#                 continue
                
#             required_keys = ['question', 'options', 'correctAnswer', 'explanation']
#             if all(key in mcq for key in required_keys):
#                 mcq['id'] = len(final_response_json) + len(valid_mcqs) + 1
...
#         if valid_mcqs:
#             final_response_json.extend(valid_mcqs)
#             print(f"Added {len(valid_mcqs)} valid MCQs from batch")
#         else:
#             error_msg = f"No valid MCQs in batch {i//batch_size + 1}"
#             print(error_msg)
#             error_messages.append(error_msg)

#     if error_messages:
#         print(f"Completed with errors: {', '.join(error_messages)}")

#     print(f"Total valid MCQs generated: {len(final_response_json)}")
#     print("Final response JSON:", final_response_json)
